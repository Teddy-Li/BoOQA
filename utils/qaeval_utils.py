...
vague_predtoks = ['是', '还是', '也是', '都是', '有', '的', 'X', "'s", "'", "has", "have", "had", "is", "are", "was", "were",
				  "of", "in", "on", "at", "be", "for", "with"]
years_set = set()


def get_auc(precisions, recalls):  # the AUC measure as with Hosseini et al. (2018)
    xs = []
    ys = []
...
def parse_time(time_str, lang):
	if lang == 'zh':
		date, time = time_str.rstrip('\n').split(' ')
		month, day = date.split('-')
		hr, mnt = time.split(':')
		return {'year': '2016', 'month': month, 'day': day, 'hour': hr, 'minute': mnt}
	elif lang == 'en':
		month_dict = {'Jan': '01', 'Feb': '02', 'Mar': '03', 'Apr': '04', 'May': '05', 'Jun': '06', 'Jul': '07',
					  'Aug': '08', 'Sep': '09', 'Oct': '10', 'Nov': '11', 'Dec': '12'}
		try:
			month, day, year, _time, _ampm = time_str.rstrip('\n').split(' ')
		except ValueError as e:
			print(f"{e}; time_str: {time_str}", file=sys.stderr)
			return {'year': '0000', 'month': '01', 'day': '01', 'hour': '12', 'minute': '00'}
		...
		hr, mnt, _ = _time.split(':')
		return {'year': year, 'month': month, 'day': day, 'hour': hr, 'minute': mnt}
...
def assemble_rel(new_upred, old_rel):
	...
	if old_rel[0] == '(':
		...
		print(f"assemble_rel: old rel does not start with '('! old_rel: {old_rel}")
	if old_rel[-1] == ')':
		old_rel = old_rel[:-1]
	else:
		print(f"assemble_rel: old rel does not end with ')'! old_rel: {old_rel}")
...
def rel2normalform(old_rel):
	"""
	ensures that when s,v,o,s_type,o_type are all the same, then the rel_str of the two relations are the same.
	:param old_rel:
...
def readWordNet(wordnet_dir):
	...
	wordnet_dict = {'lexicalEntries': lexicalEntries, 'synsets': synsets, 'sstids2lemmas': sstids2lemmas,
					'zh_en_alignments': zh_en_alignments}

	return wordnet_dict


def readWord2Vec(word2vec_path, verbose=True, lang=None):
	...
	with open(word2vec_path, 'r', encoding='utf8') as input_fp:
		...
		assert num_words is None or len(word_vectors) + len(redundant_words) == num_words
		print(f"Total number of redundant words: {len(redundant_words)}; total number of lines: {len(word_vectors)+len(redundant_words)}")
		if verbose:
			print(redundant_words)
	return word_vectors


def build_upred_vectors_h5py(all_preds, word_vectors, h5file, pred_set_path, lang):
	stopwords = ['X', '的', '【介宾】']
	...
	for type_pair in type_pairs_list:
		...
		for pred_idx, upred_str in enumerate(all_preds[type_pair]):
			...
			if all_upredstrs_global_ids[upred_str] is not None:
				...
				upred_list = [x for x in upred_str.split(' ') if len(x) > 0]
				upred_vecs_list = []
				...
				for utok in upred_list:
					...
					if utok not in word_vectors:
						# for Chinese, having OOV tokens usually means that the predicate contains some non-Chinese
						# tokens, which in turn usually means that those predicates are infelicitous; on the other hand,
						# for English, having OOV tokens is more normal, and in most cases, simply ignoring them would be
						# fine. (for instance: [john, f, kennedy, center, for, the, performing, arts, in])
						# Note: for English, when all tokens of a predicates are OOV tokens, this predicate is counted
						# as ``mismatched'' since the length of the list of vectors is zero.
						if lang in ['zh', 'en']:
							mismatch_found = True
							break
						else:
		...
		for type_pair in all_preds:
			for key in all_preds[type_pair]:
				out_line = json.dumps({'type': type_pair, 'predstr': key, 'p': all_preds[type_pair][key]['p'],
									   'n': all_preds[type_pair][key]['n'],
									   'global_id': all_preds[type_pair][key]['global_id'],
									   'lemm_predstr': all_preds[type_pair][key]['lemm_predstr'],
									   'lemm_noprep_predstr': all_preds[type_pair][key]['lemm_noprep_predstr']}, ensure_ascii=False)
				fp.write(out_line + '\n')
	return dset, mismatches_dset


def pred_trip_diff(a_str: str, b_str: str) -> bool:
	"""
	Check whether two predicates / triples are the same, that means: they are exactly the same except for the semantic role
	labels, which can be spurious.
	:param a_str: pred / triple to compare
	:param b_str: pred / triple to compare
	:return: boolean flag indicating whether the two preds / triples are different: True is different, False if same
	"""
	exempt_tokens = ['1', '2', '3', '4']
	...
	for a, b in zip(a_str, b_str):
		if a != b:
			if a in exempt_tokens and b in exempt_tokens:
				pass
			else:
				return True
...
def lemmatize_str(pred_str, lemmatizer, lang):
	# TODO: here the particle POS TAG ``RP'' should be removed to avoid splitting the particle with the verb that it leans on.
	# TODO: this is inconsistent until next iteration since re-running the do_load_triple takes a while;
	# TODO: so far this inconsistency should not matters as long as we keep setting the ``allow_prep_mismatch_replacements'' flag to False
	PREPOSITION_POS_TAGS = ['IN', 'TO']
	...
	predstr_list = [x for x in pred_str.split(' ') if len(x) > 0]
	lemm_predstr_list = []
	for tok in predstr_list:
		lemm_predstr_list.append(lemmatizer.lemmatize(tok))

	lemm_noprep_predstr_list = []
	tokens = nltk.word_tokenize(pred_str)
	tagged = nltk.pos_tag(tokens)
	for tok, postag in tagged:
		if postag not in PREPOSITION_POS_TAGS:
			lemm_noprep_predstr_list.append(lemmatizer.lemmatize(tok))
		else:
			dummy = 3 + 5

	lemm_predstr = ' '.join(lemm_predstr_list)
	lemm_noprep_predstr = ' '.join(lemm_noprep_predstr_list)

	return lemm_predstr, lemm_noprep_predstr


def load_triple_set(triples_path: str, triple_set_path: str = None, pred_set_path: str = None, max_num_lines: int = -1, verbose: bool = True,
					lang: str = None, only_preds: bool = False, only_tplstrs: bool = False) -> Tuple[dict, dict]:
	lemmatizer = WordNetLemmatizer()
	assert not only_tplstrs or not only_preds, f"only_tplstrs and only_preds cannot be both True! This would invalidify the only_tplstrs flag."

	if verbose:
		print(f"Loading triples from {triples_path}")
	all_triples = set() if only_tplstrs else dict()
	all_preds = {}
	ambiguity_count = 0
	with open(triples_path, 'r', encoding='utf8') as fp:
		for lidx, line in enumerate(fp):
			if 0 <= max_num_lines < lidx:
				break
			if lidx % 100000 == 0:
				print(f"lidx: {lidx}; ambiguity_count: {ambiguity_count}; triples_cnt: {len(all_triples)}")
			item = json.loads(line)
			for rel in item["rels"]:
				upred, subj, obj, tsubj, tobj = parse_rel(rel)
				triple_str, pred_str = rel2concise_str(upred, subj, obj, tsubj, tobj, lang=lang)
				rel_normalized = rel2normalform(rel)
				if not only_preds:
					if only_tplstrs:
						all_triples.add(triple_str)
					else:
						if triple_str not in all_triples:
							all_triples[triple_str] = {'r': rel_normalized, 'n': 0}
						elif pred_trip_diff(all_triples[triple_str]['r'], rel_normalized):
							rho = random.random()
							if verbose:
								print(f"rel_normalized mismatch: cached: {all_triples[triple_str]['r']}; current: {rel_normalized}")
							if rho < 0.3:
								all_triples[triple_str]['r'] = rel_normalized
						else:
							pass
						all_triples[triple_str]['n'] += 1

				type_pair = f"{tsubj}#{tobj}"
				if type_pair not in all_preds:
					all_preds[type_pair] = {}

				# each entry for a pred_str consists of all the upreds with their num_occ, and the total num_occ stored as ``n''
				if pred_str not in all_preds[type_pair]:
					all_preds[type_pair][pred_str] = {'p': {}, 'n': 0}

				if upred not in all_preds[type_pair][pred_str]['p']:
					all_preds[type_pair][pred_str]['p'][upred] = 0

				all_preds[type_pair][pred_str]['p'][upred] += 1
				all_preds[type_pair][pred_str]['n'] += 1
			...
			with open(triple_set_path, 'w', encoding='utf8') as fp:
				for kidx, k in enumerate(all_triples):
					...
					if only_tplstrs:
						...
						out_line = json.dumps({'tplstr': k, 'r': all_triples[k]['r'], 'n': all_triples[k]['n']},
											  ensure_ascii=False)
						fp.write(out_line + '\n')
	...
	if pred_set_path is not None:
		with open(pred_set_path, 'w', encoding='utf8') as fp:
			for type_pair in sorted(all_preds.keys()):
				print(f"Dumping {type_pair}")
				for kidx, k in enumerate(all_preds[type_pair]):
					if kidx % 200000 == 0 and kidx > 0:
						print(f"line number {kidx}")
					lemm_predstr, lemm_noprep_predstr = lemmatize_str(k, lemmatizer, lang=lang)
					cur_upreds = sorted(all_preds[type_pair][k]['p'].items(), key=lambda x: x[1], reverse=True)
					out_line = json.dumps({'type': type_pair, 'predstr': k, 'lemm_predstr': lemm_predstr,
										   'lemm_noprep_predstr': lemm_noprep_predstr, 'p': cur_upreds,
										   'n': all_preds[type_pair][k]['n']}, ensure_ascii=False)
					fp.write(out_line + '\n')
	print(f"finished.")
...
def filter_sound_triples(all_triples, global_presence_thres, global_presence_cap):
	sound_tplstrs = set()

	for tplstr in all_triples:
		if not fits_thresholds(all_triples[tplstr]['n'], global_presence_thres, global_presence_cap):
			continue
...
# cleared; untested
def reconstruct_sent_from_rel(rel, max_spansize, lang):
	upred, subj, obj, tsubj, tobj = parse_rel(rel)
	if lang == 'zh':
		assert upred[0] == '(' and upred[-1] == ')'
...
# cleared; untested
def reconstruct_sent_from_svo(upred, subj, obj, max_spansize, lang):
	if lang == 'zh':
		assert upred[0] == '(' and upred[-1] == ')'
		upred_surface_form = upred[1:-1]
		upred_surface_form = upred_surface_form.split('.1,')
...
def fits_thresholds(num_occ, thres, cap):
	if num_occ < thres:
		return False
	elif 0 < cap < num_occ:
		return False
