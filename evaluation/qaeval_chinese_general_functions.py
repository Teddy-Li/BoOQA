...
sys.path.append('/home/tianyi/multilingual-lexical-inference/lm-lexical-inference/src/data/')
...
def find_entailment_matches_from_graph(_graph, _backup_graph, _gr_predstr2preds, _bkupgr_predstr2preds, graph_deducer,
                                       ...
                                       smoothing_k: int = 4, available_graph_types: set = None,
                                       num_prems_to_check: int = None, prem_selection: str = None,):
    # print(f"find_entailment_matches_from_graph: prem_selection: {prem_selection}")
    ...
    if smooth_h is not None:
        if smooth_h == 'lm':
            if len(q_tpred_querytype.split('#')) == 3:
                ...
                q_nearest_unodes_fwd = [[[]], [[]]]
                q_nearest_unodes_rev = [[[]], [[]]]
        elif smooth_h in ['wn_hyponym', 'wn_hypernym']:
            q_upredlist = upred2bow(q_upred, lang=lang)
            smooth_direction = smooth_h.split('_')[-1]
            q_nearest_nodes = fetch_wn_smoothings(q_upredlist, direction=smooth_direction,
                                                  gr_predstr2preds=_gr_predstr2preds, first_only=True)
            if len(q_nearest_nodes[0][0]) > 0:
                ...
                q_nearest_nodes = fetch_wn_smoothings(q_upredlist, direction=smooth_direction,
                                                      gr_predstr2preds=_bkupgr_predstr2preds, first_only=True)
                q_nearest_unodes_fwd = q_nearest_nodes
                q_nearest_unodes_rev = q_nearest_nodes
                q_nnode_bkup_gr_flag = True
                q_nnode_bkup_gr_flag_fwd = True
    ...
    if prem_selection == 'scattered':
        assert num_prems_to_check is not None
        picked_up_interval = len(ref_rels) / (num_prems_to_check+1)
        ref_rels_to_use_idxes = [int(x*picked_up_interval) for x in range(1, num_prems_to_check+1)]
        ref_rels_to_use = [ref_rels[i] for i in ref_rels_to_use_idxes]
    elif prem_selection == 'topk':
        assert num_prems_to_check is not None
        ref_rels_to_use = ref_rels[:num_prems_to_check]
    else:
        assert num_prems_to_check is None and prem_selection is None, f"num_prems_to_check: {num_prems_to_check}, prem_selection: {prem_selection}"
        ref_rels_to_use = ref_rels

    for ridx, rrel in enumerate(ref_rels_to_use):
        ...
        if smooth_p is not None:
            if smooth_p == 'lm':
                if len(r_tpred_querytype.split('#')) == 3:
                    ...
                    r_nearest_unodes_fwd = [[[]], [[]]]
                    r_nearest_unodes_rev = [[[]], [[]]]
            elif smooth_p in ['wn_hypernym', 'wn_hyponym']:
                r_upredlist = upred2bow(r_upred, lang=lang)
                smooth_direction = smooth_p.split('_')[-1]
                r_nearest_nodes = fetch_wn_smoothings(r_upredlist, direction=smooth_direction, gr_predstr2preds=_gr_predstr2preds,
                                                      first_only=True)
                if len(r_nearest_nodes[0][0]) > 0:
                    ...
                    r_nearest_nodes = fetch_wn_smoothings(r_upredlist, direction=smooth_direction, gr_predstr2preds=_bkupgr_predstr2preds,
                                                          first_only=True)
                    r_nearest_unodes_fwd = r_nearest_nodes
                    r_nearest_unodes_rev = r_nearest_nodes
                    r_nnode_bkup_gr_flag = True
