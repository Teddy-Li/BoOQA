...
sys.path.append('/home/tianyi/EntGraph_nick/evaluate')
...
def qa_eval_boolean_all_partitions(args, date_slices, data_entries, entry_processed_flags, entry_tscores,
								   ...
								   entry_prm_alltype_missings=None, entry_hyp_alltype_missings=None,
	...
	# print(f"qa eval boolean all partitions, args.prem_selection: {args.prem_selection}")
	...
	if args.eval_method in ['bert1A', 'bert2A', 'bert3A']:
		...
		if args.eval_method in ['bert1A']:
			...
			sys.path.append("/home/tianyi/DrQA/")
		...
		sys.path.append('/home/tianyi/PycharmProjects/multilingual-lexical-inference/lm-lexical-inference')
		...
		sys.path.append('/home/tianyi/PycharmProjects/multilingual-lexical-inference/conan')
		...
		if args.smooth_p in ['wn_hypernym', 'wn_hyponym'] or args.smooth_h in ['wn_hypernym', 'wn_hyponym']:
	...
	for partition_key in date_slices:
		...
		for cid, ent in enumerate(cur_partition_data_entries):
			...
			if args.eval_method == 'bert1A':
				...
				cur_num_smoothing, prm_all_missing_flag, hyp_missing_flag = \
					find_entailment_matches_from_graph(gr, _backup_gr, _gr_predstr2preds, _bkupgr_predstr2preds,
													   ...
													   num_prems_to_check=args.num_prems_to_check, prem_selection=args.prem_selection,)
			...
			if cur_partition_typematched_flags[cid] is True:
				...
				if entry_sscores is not None:  # means if the smoothed scores are calculated/
					...
					assert entry_prm_missings is None and entry_hyp_missings is None

			else:
				assert cur_score is None
				assert cur_tsscore is None
				if entry_prm_alltype_missings[cur_partition_global_dids[cid]] is None:
					entry_prm_alltype_missings[cur_partition_global_dids[cid]] = prm_all_missing_flag
				else:
					entry_prm_alltype_missings[cur_partition_global_dids[cid]] &= prm_all_missing_flag
				if entry_hyp_alltype_missings[cur_partition_global_dids[cid]] is None:
					entry_hyp_alltype_missings[cur_partition_global_dids[cid]] = hyp_missing_flag
				else:
					entry_hyp_alltype_missings[cur_partition_global_dids[cid]] &= hyp_missing_flag
...
def qa_eval_boolean_main(args, date_slices):
	print(f"QA eval boolean main: args.prem_selection: {args.prem_selection}")
	...
	entry_prm_alltype_missings = [None for x in range(len(data_entries))]  # these are missings flags after smoothing
	entry_hyp_alltype_missings = [None for x in range(len(data_entries))]  # these are missings flags after smoothing
	...
	if args.eval_method == 'eg':
		...
		partition_chunk_size = 196 if args.small_memory else 456
		partition_chunks = []
		partition_ite = 0
		...
		if args.smooth_p == 'lm' or args.smooth_h == 'lm':
			...
			graph_deducer = None
			print(f"GraphDeducer not initialized!")
		...
		for dsc_idx, date_slice_chunk in enumerate(partition_chunks):
			...
			for f in files:
				...
				cur_dur_loadtriples, this_num_matches = qa_eval_boolean_all_partitions(args, date_slice_chunk, data_entries, entry_processed_flags, entry_tscores,
											   ...
											   entry_prm_alltype_missings=entry_prm_alltype_missings,
											   entry_hyp_alltype_missings=entry_hyp_alltype_missings,
		...
		total_dur_loadtriples, _ = qa_eval_boolean_all_partitions(args, date_slices, data_entries, entry_processed_flags, entry_tscores,
									   ...
									   entry_hyp_missings=None, entry_prm_alltype_missings=None,
									   entry_hyp_alltype_missings=None, gr=None, _backup_gr=None,
									   graph_deducer=None, available_graph_types=None, all_preds_set=None)
	else:
		raise AssertionError
	...
	prm_missing_cnt = 0
	hyp_missing_cnt = 0
	prm_alltype_missing_cnt = 0
	hyp_alltype_missing_cnt = 0
	for (prm_missing_flg, hyp_missing_flg, prm_alltype_missing_flg, hyp_alltype_missing_flg) in \
			zip(entry_prm_missings, entry_hyp_missings, entry_prm_alltype_missings, entry_hyp_alltype_missings):
		if args.eval_method in ['eg']:
			...
			if not (prm_alltype_missing_flg in [True, False] or args.debug):
				print(f"prm alltype missing flag anomaly!")
			if not (hyp_alltype_missing_flg in [True, False] or args.debug):
				print(f"hyp alltype missing flag anomaly!")
		...
		if hyp_missing_flg is True:
			...
			pass
		if prm_alltype_missing_flg is True:
			prm_alltype_missing_cnt += 1
		else:  # this ``else'' can be either False or None
			pass
		if hyp_alltype_missing_flg is True:
			hyp_alltype_missing_cnt += 1
		else:  # this ``else'' can be either False or None
			pass
	...
	print(f"Premise alltype missing cnt: {prm_alltype_missing_cnt} / {len(entry_prm_alltype_missings)};")
	print(f"Hypothesis alltype missing cnt: {hyp_alltype_missing_cnt} / {len(entry_hyp_alltype_missings)}")
	...
	if args.eval_method in ['bert1A', 'bert2A', 'bert3A']:
		...
		if args.num_prems_to_check is not None:
			method_ident_str += f'_numprem{args.num_prems_to_check}_{args.prem_selection}'
	elif args.eval_method in ['T51A', 'T53A']:
